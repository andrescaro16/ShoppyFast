...
func GetBestSellingProductChart(c *fiber.Ctx, db *mongo.Database) error {
	...
	var bar_items [7]opts.BarData

	for i := 0; i < len(bar_items); i++ {
		...
		if best_selling_product["_id"] == nil {
			date_to_analize_UTC = next_date_to_analize_UTC
			continue
		}

		var product_id int = int(best_selling_product["_id"].(int32))
		var product_quantity int = int(best_selling_product["cantidad"].(int32))
		...
		bar_items[i].Value = product_quantity
		bar_items[i].Name = products[0].Name + "- Cantidad vendida"
		date_to_analize_UTC = next_date_to_analize_UTC
	}
